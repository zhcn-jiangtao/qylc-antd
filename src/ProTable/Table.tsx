...
const ProTable = <T extends {}, U extends object>(
    ...
    if (rest.onChange == null) {
        rest.onChange = (pagination, filters, sorter, extra) => {
            ...
            if (field) {
                if (order) {
                    let _orderBy = `${field},${order == 'ascend' ? 'asc' : 'desc'}`;
                    setQueryParam({orderBy: _orderBy})
                } else {
     ...
     * 为了保证不会重复刷新
     */
    const fetchPagination = typeof propsPagination === 'object' ? (propsPagination as TablePaginationConfig) :
    ...
    const action = useFetchData(
        async ({pageSize, current}) => {
            ...
            const rs = await request({pageNumber: current, pageSize, ...formSearch, ...params, ...queryParam} as U);
        ...
        defaultData,
        {
            ...
            onRequestError,
            // 指定哪些属性变化，会触发函数
            effects: [stringify(params), stringify(formSearch), stringify(queryParam)],
        },
    );
